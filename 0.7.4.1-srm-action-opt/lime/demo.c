/* 
   v0.7.4.1 - 3/12/2015 @author Tae Seung Kang
   Spike Reponse Model (SRM) with continuous force version
...
#define NSTEPS		3600000 // 3.6M or 3600k * 8 bytes = 28.8MB
//#define IMPULSE	
...
double r_hat[2], push, unusualness[2], fired[2], pushes[NSTEPS], forceValues[100];
double last_spike_p[2][NSTEPS], last_spike_x[5][NSTEPS], last_spike_v[5][NSTEPS], last_spike_z[5][NSTEPS];
...
void action(int step) {
  ...
  for(i = 0; i < 5; i++)
	...
	//sum += d[i][j] * x[j];
      //z[i] = 1.0 / (1.0 + exp(-sum));
        for(k = 1; k < 100; k ++) 
  	  if(last_spike_x[j][step - k] != 0) 
	    sum += d[j][i] * PSP(k);
      }
      for(k = 1; k < 20; k ++) 
  	if(last_spike_z[i][step - k] != 0) 
	  sum += AHP(k);
      z[i] = sum;
      if (z[i] >= 1.0) {
	last_spike_z[i][step] = 1;
